...
- `무공변(invariant)`
제네닉 타입을 인스턴스화할 때 타입 인자로 서로 다른 타입이 들어가면 인스턴스 타입 사이의 `하위 타입 관계`가 성립하지 않기 때문에 이를 가리켜 `무공변(invariant)`이라고 한다. 

- `공변(covariant)`
만약 A타입이 B타입의 하위 타입이면 `List<A>`는 `List<B>`의 하위 타입이다. 이런 `클래스` 혹은 `인터페이스`를 가리켜 `공변(covariant)`적이라고 말한다. 

- 코틀린에서는 공변적이 성격을 표시하는 문법적인 요소로 out 키워드를 사용하여 이를 표현할 수 있으며, 이 경우 클래스 내부에서 그 파라미터를 사용하는 방법을 제한하게 된다. 항상 아웃 위치에만 있어야 하는데 이는 다시 말해 `T타입의 값을 생산(반환)할 수 있지만, 소비(인자)할 수는 없다는 이야기이다.`
```kotlin
